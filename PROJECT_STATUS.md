...
> 마지막 갱신: 2026-07-13 (**명소 기능 라이브 + 글 재작성/예약발행** `80b20b1`. 7곳 여행친구 톤 재작성(웹리서치·자체창작)·예약발행 ISR(미래 publishedAt=공개일 자동 등장). MVP+Neon DB+TourAPI 7곳. 라이브 workmate.tools/attractions·댓글 DB·ads.txt 200. 프로젝트 최초 DB·API route·next/image·ISR. 이전: Phase 4 breadcrumb)
...
정본: [`docs/strategy-global-ax.md`](./docs/strategy-global-ax.md) — "계산기 모음 → Korea OS + AI가 인용·호출하는 한국 표준 데이터원" 재정의.

**Phase A 완료 (2026-07-03, `ce5538a`)**: `/data` 신선도 레지스트리(12항목, 계산기와 동일 상수 import) + `llms.txt`·`llms-full.txt`(GEO, 동적 생성) + audit [J] 만료 경고 + AI referral 가이드. **구축 즉시 stale 3건 실발견·수정** — 최저임금 2026 10,320원 / 국민연금 상·하한 659만·41만(2026.7~) / 실업급여 68,100·66,048원. 남은 마스터 액션: AdSense 재신청 · GSC 색인(11 URL) · Reddit 시딩 · 월 1회 GA4 AI referral 확인.
- **3링 TAM**: 거주 외국인 278만(법무부) → 한국행 예정자(유학 31만·E-9 34만) → K-컬처 글로벌 팬덤
- **6축**: 여정 커버리지 / 다국어(vi 1순위 — 등록외국인 18.4%·유학생 1위·송금 CPA 정합) / AX(llms.txt·/data 신선도 레지스트리·Korea Standards MCP API) / 유통(임베드·FB 커뮤니티·링커블 리포트) / 수익 사다리 / 신뢰 인프라
- **금지**: 전량 번역, 무상태 변환기 신규, 승인 전 제휴, 실시간 환율 인용
- **90일**: Phase A(llms.txt+/data+재신청) → B(vi TOP8+MCP MVP) → C(측정 기반 확장)

## 🧭 전략 확정 (2026-07-02) — GSC 국가별 데이터 = 영어 우선

**근거**: GSC 3개월 데이터에서 **미국 트래픽이 한국보다 검색순위 우위**(US 평균 9.2위 vs 한국 14.1위), 영어+한국특화 로직 도구(school-grade 20클릭/531노출/5.8위)가 검증된 승자.
변환 계산기(percent·distance·pyeong)는 구글 인라인 답변 위젯 탓에 CTR 구조적 저하 → 순위 미세조정보다 **영어 콘텐츠·유통 확대**가 지렛대.

**외국인 고통점 도구 3종 신규** (`ec22626`) — 최대 고통 검색의도 공략, 영어 우선. **정확성 원칙**: 모든 추정치에 "범위·기준일·법 아님" 라벨 + 강한 면책 + 공식출처·직접확인 안내. 과장·확정 표현 금지.
- **`deposit-risk` 깡통전세 위험도** (korea group 아님, realestate) — 부채비율=(선순위+보증금)/시세, 경매 회수·손실 시뮬. jeonse-wolse(정성 체크리스트)와 **중복 회피 위해 정량 계산기로 설계** → 상호 링크. AdSense 거절사유 #4(중복본문) 방지.
- **`cost-of-living` 생활비** (korea) — 지역×가구 편집가능 기본값 → 합계=본인 예산(단정 아님). USD 배제(환율변동 = 허위 위험).
- **`remittance` 해외송금** (korea) — 환율마진+수수료 실질비용률 교육. 특정업체·실시간환율 배제, 규정은 "구조+직접확인" 프레임.
- 검증: 577 테스트(19 신규)·build 302/302·audit(i18n parity·thin0·coming-soon0)·브라우저 QA ko/en(콘솔0·인터랙션·계산 일치)·배포·IndexNow 6 URL.

**직전 작업** (`d83dcae`):
- **신규 pillar 블로그** `korean-units-numbers-for-foreigners` (영어 우선) — 평·나이·사이즈·거리·온도·사업자번호를 한 글에 집약, 외국인 도구 9종 허브 링크. 필러가 아니라 "허브형 진입점".
- **도구 4종 SEO 재배선**: area-convert=순수 평 변환 쿼리 소유 / apartment-area=전용·공급·평당가 부동산 니치로 **재차별화(평 키워드 자가잠식 해소)** / school-grade=출생연도→학년 / biznum-check=1-3-7 체크섬.
- **수익 전략 로드맵 정립** (`66b8767`, `docs/monetization-strategy.md`): 정직한 3-Track — AdSense=바닥, 금융 제휴/CPA=주력 상방(AdSense 통과 후), 트래픽=병목. 휴면 OfferSlot 인프라(비노출=AdSense 심사 무해) 배선 완료.

다음 규율: **AdSense 재신청 + GSC 색인 요청 + 영문 커뮤니티 노출로 실제 트래픽 신호 검증**. 제휴 링크 실제 노출은 AdSense 통과 후.
