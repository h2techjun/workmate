...
## 🆕 2026-05-30 추가 조사 — 외국인·한국 학습/생활 도구 (검색 근거)
...
| 도구 | 검색 근거 | 비고 |
|---|---|---|
| **K-pop 굿즈/앨범 지출 계산기** | explorekoreanow 등 fandom 계산기 인기 | 앨범 버전·포카·콘서트 비용 합산 |
| **콘서트·덕질 예산 계산** | 팬덤 바이럴 SEO | 위와 묶음 |

### 권장 진행 순서
1. 거리 단위 변환 + 한글 로마자(일반) + 온도 — 외국인 필수 묶음
2. 한글 음절 분해 + 한국 학년 — 학습 묶음
3. 한국 공휴일 — 연도별 (양력 기준 먼저, 음력은 데이터 확보 후)
4. K-pop 지출 — 바이럴 엔게이지먼트

> 음력 의존 도구(명절 날짜·음력 생일)는 KASI 데이터 검증 후 별도 진행.
