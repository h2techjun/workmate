...
export async function generateMetadata({
  ...
  const c = KOREAN_CROSSWORD_COPY[localeKeyOf(locale)];
  return {
    title: `${c.metaTitle} — ${SITE_BRAND}`,
    description: c.metaDesc,
    keywords: [...c.keywords],
    alternates: {
      canonical: `/${locale}/korean-crossword`,
      languages: buildLanguagesAlt("/korean-crossword"),
    },
    openGraph: {
      title: `${c.metaTitle} — ${SITE_BRAND}`,
      description: c.metaDesc,
      url: `${SITE_URL}/${locale}/korean-crossword`,
      type: "website",
    },
  };
}
...
export default async function KoreanCrosswordPage({
  params,
}: PageProps): Promise<React.ReactElement> {
  const { locale } = await params;
  const lk = localeKeyOf(locale);
  const c = KOREAN_CROSSWORD_COPY[lk];
  ...
  return (
    <main className="px-4 pb-16 pt-6 md:px-6 md:pt-10">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {/* 게임 임베드 — 아케이드 카드 안에 kword SPA(iframe) */}
        <section
          style={{ ["--card-accent" as string]: "#22d3ee" }}
        ...
        <section className="mt-12 border-t border-[color:var(--color-border-subtle)] pt-10">
          ...
          <Link
            href={`/${locale}/games`}
            className="group mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-cyan-400 transition-colors hover:text-cyan-300"
          >
            {c.gamesCta}
