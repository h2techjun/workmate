...
export async function generateMetadata({
  ...
  const title = isKo
    ? "전용·공급면적 / 평당가 계산기 — 84㎡는 실제 몇 평 거주?"
    : "Korea Apartment Area & Price-per-Pyeong — Exclusive vs Supply";
  const description = isKo
    ? "공급면적·전용면적(㎡)을 입력하면 평 환산·전용률·평당가(공급/전용 기준)를 즉시 계산. 외국인이 가장 많이 헷갈리는 전용/공급면적 함정 해설. 1평=3.3058㎡."
    : "Enter supply and exclusive area (㎡) to get pyeong, exclusive ratio, and price per pyeong. Decodes the exclusive-vs-supply area trap that confuses every foreign renter. 1 pyeong = 3.3058㎡.";
  const keywords = isKo
    ? [
        "전용면적 공급면적",
        "전용률 계산",
    ...
    : [
        "korea exclusive area supply area",
        "korea apartment price per pyeong",
        "exclusive vs supply area korea",
        "korean apartment size explained",
        "84 sqm to pyeong",
        "korea apartment exclusive ratio",
      ];
