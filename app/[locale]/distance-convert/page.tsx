...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title, description, keywords, alternates: { canonical: `/${locale}/distance-convert`, languages: buildLanguagesAlt("/distance-convert") }, openGraph: { title, description, type: "website", url: `${SITE_URL}/${locale}/distance-convert`, locale: locale === "ko" ? "ko_KR" : locale === "vi" ? "vi_VN" : "en_US" } };
}
    ...
    <main className="px-4 pb-16 pt-6 md:px-6 md:pt-10">
      <div className="mx-auto max-w-6xl">
        <nav className="mb-5 flex items-center gap-2 text-sm text-[color:var(--color-text-tertiary)]">
          <Link href={`/${locale}/tools`} className="inline-flex items-center gap-1 transition-colors hover:text-[color:var(--color-text-primary)]"><ChevronLeft className="h-4 w-4" />{isKo ? "툴 모음" : isVi ? "Tất cả công cụ" : "All tools"}</Link>
        </nav>
        <header className="mb-8">
          <h1 className="text-2xl font-bold tracking-tight md:text-3xl">{isKo ? "거리 단위 변환 (리·자)" : isVi ? "Chuyển đổi khoảng cách (ri·ja)" : "Korean Distance Converter"}</h1>
          <p className="mt-2.5 max-w-3xl text-sm leading-relaxed text-[color:var(--color-text-secondary)] md:text-base">{isKo ? "한국 전통 거리 단위(리·자·보)를 km·미터·마일로 즉시 변환. 십리가 몇 km인지 한 번에." : isVi ? "Chuyển đổi đơn vị khoảng cách truyền thống Hàn Quốc (ri, ja, bo) sang km, mét và dặm tức thì. Xem ngay mười ri là bao nhiêu km." : "Convert Korean traditional distance units to km, meters, and miles in one place."}</p>
        </header>
