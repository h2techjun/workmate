...
export async function generateMetadata({
  ...
  if (locale === "zh") {
    return {
      ...
      description:
        "只看KEC 232.5表格一行数据来定电线截面积,现场很容易出现跳闸或过热事故,后期返工成本很高。本文从锅炉房实际跳闸案例,到变频器电路的隐藏陷阱、温度与回路修正系数,汇总多年现场经验整理成实务指南。",
    };
  }
  if (locale === "vi") {
    return {
  ...
  return {
    ...
    description:
      "KEC 232.5 표 한 줄만 보고 전선 단면적을 정하면 현장에서 트립·과열 사고로 이어집니다. 보일러실에서 실제 트립이 났던 사례부터 인버터 회로의 함정, 온도·회로 보정계수까지 현장 경력을 묶어 정리한 실무 가이드입니다.",
  };
}
