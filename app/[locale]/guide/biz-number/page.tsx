...
export async function generateMetadata({
  ...
  if (locale === "ko") {
    return {
      ...
      description:
        "104-81-XXXXX와 104-50-XXXXX는 무엇이 다를까요. 가운데 두 자리로 사업자 종류(법인·개인·비영리 등)를 알아보는 법과, 마지막 자리 체크섬으로 전화 한 통 안 걸고 가짜 번호를 잡아내는 검증법을 Q&A로 정리했습니다.",
    };
  }
  if (locale === "zh") {
    return {
      ...
      description:
        "104-81-XXXXX与104-50-XXXXX有什么区别?本文教你用中间两位数字判断事业者种类(法人·个人·非营利等),并通过最后一位校验位,不打电话也能识破虚假号码,以问答形式整理讲解，签约或开票前先自查一遍。",
    };
  }
  if (locale === "vi") {
    return {
