...
export async function generateMetadata({
  ...
  if (locale === "ko") {
    return {
      ...
      description:
        "주거용 2x6~2x12 SPF #2 부재가 실제로 안전하게 갈 수 있는 거리를 정리했습니다. 휨과 처짐 중 어느 쪽이 먼저 한계에 닿는지, 강원 영동 지역의 적설하중이 왜 설계를 완전히 바꾸는 게임 체인저인지 실제 시공 사례로 설명합니다.",
    };
  }
  if (locale === "zh") {
    return {
      ...
      description:
        "整理住宅用2x6~2x12 SPF #2构件实际能够安全跨越的距离。通过真实施工案例说明,弯曲与挠度究竟哪一个先达到极限,以及江原岭东地区的积雪荷载为何会彻底改变设计方案,是真正的变数，直接影响材料成本与安全性。",
    };
  }
  if (locale === "vi") {
    return {
